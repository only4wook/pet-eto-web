    ...
    <section style={{ padding: "clamp(44px, 6vw, 72px) 0", background: "#FFFDF9" }}>
      <div className="container-pet">
        <div style={{ textAlign: "center", marginBottom: 22 }}>
          ...
          <h2 className="text-display-md headline-balance" style={{ margin: "12px 0 8px" }}>
            모두를 위한 서비스가 아니라,
            <br />
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 14 }} className="target-grid">
          {TARGETS.map((t) => (
            <article key={t.title} style={{ background: "#fff", border: "1px solid #F3F4F6", borderRadius: 14, padding: 16 }}>
              <h3 className="headline-balance" style={{ margin: "0 0 8px", fontSize: 16, fontWeight: 800, color: "#1D1D1F" }}>{t.title}</h3>
              <p className="readable-kor" style={{ margin: "0 0 8px", fontSize: 13, color: "#6B7280" }}>문제: {t.pain}</p>
              <p className="readable-kor" style={{ margin: 0, fontSize: 13, color: "#374151" }}>해결: {t.solution}</p>
            </article>
          ))}
        </div>
      </div>
