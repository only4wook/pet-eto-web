...
export default function DogBreedPage({ params }: { params: Promise<{ breed: string }> }) {
  ...
  if (!breed) {
    return (<><Header /><main style={{ maxWidth: 900, margin: "0 auto", padding: 40, textAlign: "center" }}>
      ...
      <Link href="/wiki/dog" style={{ color: "#2EC4B6" }}>← 강아지 위키로</Link>
    </main><Footer /></>);
  }
          ...
          <div style={{ padding: 24 }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(140px, 1fr))", gap: 8, marginTop: 16, marginBottom: 24 }}>
              {[
                ...
                <div key={i} style={{ background: "#F0FFFE", borderRadius: 8, padding: "10px 14px" }}>
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginBottom: 24 }}>
              {breed.personality.map((p, i) => (
                <span key={i} style={{ background: "#2EC4B6", color: "#fff", padding: "4px 12px", borderRadius: 16, fontSize: 12, fontWeight: 600 }}>{p}</span>
              ))}
            </div>
            ...
            {[
              ...
              <div key={i} style={{ marginBottom: 20 }}>
                <h3 style={{ fontSize: 15, fontWeight: 700, borderBottom: "2px solid #2EC4B6", paddingBottom: 6, marginBottom: 8 }}>
                  {section.title}
                </h3>
