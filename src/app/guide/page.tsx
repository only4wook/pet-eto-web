    ...
    <>
      ...
      <main style={{ maxWidth: 800, margin: "0 auto", padding: "20px 16px", flex: 1 }}>
        ...
        <section style={{
          ...
          padding: "32px 28px", color: "#fff", textAlign: "center", marginBottom: 20,
        }}>
          <h1 style={{ fontSize: 26, fontWeight: 900, margin: "0 0 6px" }}>P.E.T 이용 가이드</h1>
          <p style={{ fontSize: 14, opacity: 0.9, margin: 0 }}>안심하고 맡기세요. 모든 과정이 투명합니다.</p>
        </section>
...
function CustomerGuide() {
  return (
    <>
      {/* 안전/안심 안내 (최상단 — 가장 중요) */}
      <section style={{ background: "#fff", borderRadius: 16, padding: "28px", marginBottom: 20, border: "2px solid #059669" }}>
        <h2 style={{ fontSize: 18, fontWeight: 800, color: "#059669", margin: "0 0 16px" }}>🛡️ 안심하고 맡기세요</h2>

        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(200px, 1fr))", gap: 12, marginBottom: 20 }}>
          {[
            { icon: "🔍", title: "3단계 파트너 검증", desc: "신원조회 → 대면/화상 면접 → 시범 케어 통과한 분만 활동합니다." },
            { icon: "🔒", title: "현관문 출입 안심", desc: "일회용 비밀번호 설정 안내, 케어 종료 후 비밀번호 변경 가이드를 제공합니다." },
            { icon: "📸", title: "실시간 사진 보고", desc: "케어 중 사진/영상을 카카오톡으로 실시간 전송합니다." },
            { icon: "🛡️", title: "사고 발생 시 보상", desc: "케어 중 발생한 사고는 P.E.T가 중재하며, 배상책임보험 가입을 추진 중입니다." },
          ].map((item, i) => (
            <div key={i} style={{ background: "#ECFDF5", borderRadius: 12, padding: 16 }}>
              <div style={{ fontSize: 24, marginBottom: 6 }}>{item.icon}</div>
              <div style={{ fontSize: 14, fontWeight: 700, color: "#065F46" }}>{item.title}</div>
              <div style={{ fontSize: 12, color: "#047857", marginTop: 4, lineHeight: 1.6 }}>{item.desc}</div>
            </div>
          ))}
        </div>
        ...
        <div style={{ background: "#F0FDF4", borderRadius: 12, padding: 16 }}>
          <h3 style={{ fontSize: 14, fontWeight: 700, color: "#065F46", margin: "0 0 8px" }}>🏠 현관문 출입은 이렇게 진행됩니다</h3>
          <div style={{ fontSize: 13, color: "#047857", lineHeight: 1.8 }}>
            1. 매칭 확정 시 <strong>일회용 비밀번호</strong>를 설정해주세요 (도어락 설정 가이드 제공)<br />
            2. 파트너에게 일회용 비밀번호만 전달 (기존 비밀번호 노출 X)<br />
            3. 케어 완료 후 <strong>비밀번호 즉시 변경</strong> (가이드 안내)<br />
            4. 파트너는 P.E.T 규정에 따라 비밀번호 즉시 삭제 의무
          </div>
        </div>
      </section>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(160px, 1fr))", gap: 12 }}>
          {[
            ...
            <div key={i} style={{ textAlign: "center" }}>
              <div style={{
                width: 48, height: 48, borderRadius: 14, background: "#FF6B35",
                display: "flex", alignItems: "center", justifyContent: "center",
                fontSize: 22, margin: "0 auto 8px",
              }}>{s.icon}</div>
              <div style={{ fontSize: 11, fontWeight: 700, color: "#FB923C" }}>STEP {s.step}</div>
              <div style={{ fontSize: 14, fontWeight: 700, color: "#1F2937" }}>{s.title}</div>
      ...
      <section style={{ background: "#fff", borderRadius: 16, padding: "28px", marginBottom: 20, border: "1px solid #F3F4F6" }}>
        ...
        <div style={{ fontSize: 13, color: "#6B7280", lineHeight: 1.8 }}>
          <p style={{ margin: "0 0 8px" }}>• 서비스 이용 시 <strong style={{ color: "#FF6B35" }}>결제액의 3%</strong> 자동 적립</p>
          <p style={{ margin: "0 0 8px" }}>• 후기 작성 시 <strong style={{ color: "#FF6B35" }}>+500~1,000P</strong> (사진 포함 시 더 많이!)</p>
          <p style={{ margin: "0 0 8px" }}>• 친구 초대 시 <strong style={{ color: "#FF6B35" }}>+3,000P</strong> (추천인/피추천인 모두)</p>
          <p style={{ margin: "0 0 8px" }}>• 결제 시 최대 10% 포인트 할인 가능 (1P = 1원)</p>
        </div>
      </section>

      {/* CTA */}
      <section style={{ background: "linear-gradient(135deg, #FFF7ED, #FEF3C7)", borderRadius: 16, padding: "28px", textAlign: "center", marginBottom: 20 }}>
    ...
    <>
      ...
      <section style={{ background: "#fff", borderRadius: 16, padding: "28px", marginBottom: 20, border: "1px solid #F3F4F6" }}>
        ...
        <div style={{ background: "#FFF7ED", borderRadius: 12, padding: 20, marginBottom: 16 }}>
          <h3 style={{ fontSize: 15, fontWeight: 700, color: "#92400E", margin: "0 0 10px" }}>💡 정산 예시 (긴급 방문 1시간 30,000원)</h3>
          <div style={{ fontSize: 13, color: "#6B7280", lineHeight: 2 }}>
            고객 결제: <strong style={{ color: "#1F2937" }}>30,000원</strong> (1시간)<br />
            → PG 수수료 (3.5%): -1,050원<br />
            → P.E.T 수수료 (21.5%): -6,450원<br />
            → 파트너 지급 대상: 22,500원<br />
            → 원천징수 (3.3%): -743원<br />
            → <strong style={{ color: "#059669", fontSize: 15 }}>파트너 실수령: 21,757원 (시급 약 21,700원)</strong>
          </div>
        </div>
      </section>
      ...
      <section style={{ background: "#fff", borderRadius: 16, padding: "28px", marginBottom: 20, border: "1px solid #F3F4F6" }}>
        ...
        <div style={{ fontSize: 13, color: "#6B7280", lineHeight: 1.8 }}>
          ...
          <p style={{ margin: 0, background: "#DBEAFE", padding: "8px 12px", borderRadius: 8 }}>
      ...
      {/* 파트너 신청 CTA */}
      <section style={{ background: "linear-gradient(135deg, #FFF7ED, #FEF3C7)", borderRadius: 16, padding: "28px", textAlign: "center" }}>
