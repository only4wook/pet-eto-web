...
// 자동 생성된 닉네임 패턴 감지: 사전 형용사 + 명사 + 4자리 숫자
// (예: "느긋한펫맘4839", "씩씩한펫대디5522")
// 사용자가 직접 고른 닉네임은 통과하고, 시스템 자동 닉네임만 잡아냄.
export function isAutoGeneratedNickname(nickname: string | null | undefined): boolean {
  if (!nickname) return false;
  const n = String(nickname).trim();
  // 자동 생성 형식: ADJ + NOUN + 4자리 숫자 (1000~9999)
  for (const adj of NICKNAME_ADJ) {
    for (const noun of NICKNAME_NOUN) {
      if (new RegExp(`^${adj}${noun}\\d{4}$`).test(n)) return true;
    }
  }
  return false;
}
